...
using Elmah;
using DeaconCCGManagement.Elmah;
...
namespace DeaconCCGManagement.DeaconAnnouncements
{
    public class AnnoucementRepository
    {
        ...
        {
            if (!Init()) return new List<AnnouncementModel>();

            var announements = new List<AnnouncementModel>();
            ...
            announements = _table.ExecuteQuery(query).ToList();
